...
async def profile_async_search(
    test_vespa: VespaSearchAdapter, params: Mapping[str, str], n: int = 25
) -> float:
    t = timeit(
        lambda: async_vespa_search(test_vespa, SearchParameters(**params)),
        number=n,
    )
    avg_ms = (t / n) * 1000
    return avg_ms
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__works(test_vespa):
    request = SearchParameters(query_string="the")
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor_relevance_scoring(test_vespa):
    request = SearchParameters(query_string="the")
    response = await async_vespa_search(test_vespa, request)
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor_rank_features(test_vespa):
    request = SearchParameters(query_string="the")
    response = await async_vespa_search(test_vespa, request)
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__exact_search(test_vespa):
    """Test that exact search works"""
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__is_fast_enough(test_vespa, params):
    MAX_SPEED_MS = 850

    avg_ms = await profile_async_search(test_vespa, params=params)
    assert avg_ms <= MAX_SPEED_MS
...
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__document_ids(test_vespa, document_ids):
    request = SearchParameters(query_string="the", document_ids=document_ids)
    response = await async_vespa_search(test_vespa, request)
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__bad_query_string_still_works(test_vespa):
    family_name = ' Bad " query/    '
    request = SearchParameters(query_string=family_name)
    try:
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__hybrid(test_vespa):
    family_name = "Nationally Determined Contribution: Climate Change Adaptation and Low Emissions Growth Strategy by 2035"
    request = SearchParameters(query_string=family_name)
    response = await async_vespa_search(test_vespa, request)
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__exact(test_vespa):
    query_string = "Environmental Strategy for 2014-2023"
    request = SearchParameters(query_string=query_string, exact_match=True)
    response = await async_vespa_search(test_vespa, request)
    got_family_names = []
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__limits(
    test_vespa, family_limit, max_hits_per_family
):
    request = SearchParameters(
        query_string="the",
        family_ids=[],
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__continuation_tokens__families(test_vespa):
    query_string = "the"
    limit = 2
    max_hits_per_family = 3
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__continuation_tokens__passages(test_vespa):
    query_string = "the"
    limit = 1
    max_hits_per_family = 10
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__continuation_tokens__families_and_passages(
    test_vespa,
):
    query_string = "the"
    limit = 1
    ...
    response_one = await async_vespa_search(test_vespa, request_one)
    ...
    response_two = await async_vespa_search(test_vespa, request_two)
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_adapter_sorting(test_vespa, sort_by):
    ascend = await async_vespa_search(
        test_vespa,
        SearchParameters(query_string="the", sort_by=sort_by, sort_order="ascending"),
    )
    descend = await async_vespa_search(
        test_vespa,
        SearchParameters(query_string="the", sort_by=sort_by, sort_order="descending"),
    )
...
@pytest.mark.vespa
@pytest.mark.asyncio
async def test_vespa_async_search_no_passages_search(test_vespa):
    no_passages = await async_vespa_search(
        test_vespa,
        SearchParameters(all_results=True, documents_only=True),
    )
    for family in no_passages.families:
    ...
    with_passages = await async_vespa_search(
        test_vespa,
        SearchParameters(all_results=True),
    )
    found_a_passage = False
...
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__corpus_type_name(
    test_vespa, query_string, corpus_type_names
):
    """Test that the corpus type name filter works"""
    request = SearchParameters(
...
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__concept_filter(
    test_vespa, concept_filters: list[dict]
):
    """Test that the concept filter works"""
    request = SearchParameters(
        all_results=True,
...
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__corpus_import_ids(
    test_vespa, query_string, corpus_import_ids
):
    """Test that the corpus import ids filter works"""
    request = SearchParameters(
...
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__metadata(
    test_vespa, query_string, metadata_filters
):
    """Test that the metadata filter works"""
    request = SearchParameters(
        query_string=query_string,
...
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__filters(test_vespa, query_string, filters):
    """Test that the search filters work"""
    request = SearchParameters(
        query_string=query_string,
        filters=Filters.model_validate(filters),
...
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__concept_counts(
    test_vespa,
    concept_count_filters: list[ConceptCountFilter],
    expected_response_families: set[str],
    sort_by: Union[str, None],
...
@pytest.mark.asyncio
async def test_vespa_async_search_adaptor__concept_counts_with_other_filters(
    test_vespa,
    search_parameters: SearchParameters,
) -> None:
